...
for i in range(1, 11, 1):
    ...
    for line in open(f"C:\\Users\\joaov\\Documents\\GitHub\\documents_msc\\Dados\\testes_mestrado\\teste_cenario_modelo_visao_quad_maior\\teste_cenario_modelo_visao_quad_maior\\ekf\\ekf_cenario_modelo_visao_quad_maior_{i:02d}.txt", 'r'):
    # for line in open(f"C:\\Users\\joaov\\Documents\\GitHub\\documents_msc\\Dados\\testes_mestrado\\teste_cenario_modelo_visao_quad_menor_2\\teste_cenario_modelo_visao_quad_menor_2\\ekf\\ekf_cenario_modelo_visao_quad_menor_{i}.txt", 'r'):
        lines_ekf.append(line.rstrip().split(";"))

    tamanho_ekf = len(lines_ekf[1:])
    print(f"Quantidade amostras: {tamanho_ekf}")

    for data in lines_ekf[int(0.25*tamanho_ekf):int(0.85*tamanho_ekf)]:
        ...
        vel_x_theo.append(float(data[5])/1000.0)
        vel_y_theo.append(float(data[6])/1000.0)
        vel_theta_theo.append(float(data[7])) #*np.pi/180.0)

        vel_x_encoder.append(float(data[8])/1000.0)
        vel_y_encoder.append(float(data[9])/1000.0)
        vel_theta_encoder.append(float(data[10])) #*np.pi/180.0)

    for line in open(f"C:\\Users\\joaov\\Documents\\GitHub\\documents_msc\\Dados\\testes_mestrado\\teste_cenario_modelo_visao_quad_maior\\teste_cenario_modelo_visao_quad_maior\\laser\\laser_cenario_modelo_visao_quad_maior_{i:02d}.txt", 'r'):
    # for line in open(f"C:\\Users\\joaov\\Documents\\GitHub\\documents_msc\\Dados\\testes_mestrado\\teste_cenario_modelo_visao_quad_menor_2\\teste_cenario_modelo_visao_quad_menor_2\\laser\\laser_cenario_modelo_visao_quad_menor_{i}.txt", 'r'): 
    ...
    print(f"Std Dev error laser: {np.std(errors_laser):.5f} m".replace('.', ','))
    print(f"Mean error x laser: {np.sqrt(np.mean(errors_x_laser)):.5f} m".replace('.', ','))
    print(f"Std Dev error x laser: {np.std(errors_x_laser):.5f} m".replace('.', ','))
    print(f"Mean error y laser: {np.sqrt(np.mean(errors_y_laser)):.5f} m".replace('.', ','))
    print(f"Std Dev error y laser: {np.std(errors_y_laser):.5f} m".replace('.', ','))
    ...
    print(f"Std Dev error vision: {np.std(errors_vision):.5f} m".replace('.', ','))
    print(f"Mean error x vision: {np.sqrt(np.mean(errors_x_vision)):.5f} m".replace('.', ','))
    print(f"Std Dev error x vision: {np.std(errors_x_vision):.5f} m".replace('.', ','))
    print(f"Mean error y vision: {np.sqrt(np.mean(errors_y_vision)):.5f} m".replace('.', ','))
    print(f"Std Dev error y vision: {np.std(errors_y_vision):.5f} m".replace('.', ','))
    ...
    print(f"Std Dev error theta: {np.std(errors_theta_vision):.5f} degrees".replace('.', ','))
    ...
    # print(f"{np.sqrt(np.mean(errors_laser)):.5f};{np.std(errors_laser):.5f};{np.sqrt(np.mean(errors_x_laser)):.5f};{np.std(errors_x_laser):.5f};{np.sqrt(np.mean(errors_y_laser)):.5f};{np.std(errors_y_laser):.5f};{np.sqrt(max(errors_laser)):.5f};{np.sqrt(np.mean(errors_vision)):.5f};{np.std(errors_vision):.5f};{np.sqrt(np.mean(errors_x_vision)):.5f};{np.std(errors_x_vision):.5f};{np.sqrt(np.mean(errors_y_vision)):.5f};{np.std(errors_y_vision):.5f};{np.sqrt(max(errors_vision)):.5f};{np.sqrt(np.mean(errors_theta_vision)):.5f};{np.std(errors_theta_vision):.5f};{np.sqrt(max(errors_theta_vision)):.5f};{tamanho_ekf}".replace('.', ','))
    ...
    plt.figure()
    plt.plot(range(len(vel_theta_encoder)), vel_theta_encoder, c='b', label='Encoder')
    plt.plot(range(len(vel_theta_theo)), vel_theta_theo, c='g', label='Teórica')
    ...
    plt.plot(range(len(vel_x_theo)), vel_x_theo, c='g', label='Teórica')
    ...
    plt.plot(range(len(vel_y_theo)), vel_y_theo, c='g', label='Teórica')
