	...
	\section{FILTRO DE KALMAN}
		...
		\subsection{Equações}
			...
			\subsubsection{Predição}
				...
				No caso, $bel(x_k) = N(\hat{x}_{k}^{-},P_{k}^{-})$, em que a média $\hat{x}_{k}^{-}$ e a covariância $P_{k}^{-}$ são definidos segundo a Equação \eqref{eq:equacao_predicao_kf}.
			...
			\subsubsection{Correção}
				...
				A nova crença posterior é utilizada no próximo passo de tempo para calcular a nova crença. A natureza recursiva do filtro de Kalman permite implementações práticas, já que nem todos os dados são necessários para estimar os estados.
